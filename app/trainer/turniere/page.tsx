    ...
    <div className="space-y-5">
      ...
      {!loading && tournaments.length > 0 && (
        <div className="space-y-2">
          {tournaments.map((t) => <TournamentItem key={t.id} t={t} />)}
        </div>
      )}
    </div>
  )
...
function TournamentItem({ t }: { t: TournamentWithCount }) {
  return (
    <Link href={`/trainer/turnier/${t.id}`} className="flex items-center justify-between gap-3 rounded-xl border border-border/60 px-4 py-3.5 hover:border-border transition-colors">
